...
def test_open_draft_pr_uses_configured_api_base(monkeypatch):
    ...
    monkeypatch.setattr(github_client, "GITHUB_REPO", "autofix-self-healing-pipeline")
    captured = {}

    class FakeResp:
        def raise_for_status(self): pass
        def json(self): return {"number": 5, "html_url": "https://github.cainc.com/hackathon/autofix-self-healing-pipeline/pull/5"}
    ...
    assert captured["url"] == "https://github.cainc.com/api/v3/repos/hackathon/autofix-self-healing-pipeline/pulls"
    assert num == 5
...
def test_commit_fix_and_open_pr_pushes_then_opens(monkeypatch):
    ...
    assert calls[0][1].startswith("AutoFix-KR-9-")
