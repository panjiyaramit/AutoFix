...
def test_summary_fallback_when_empty():
    assert jira_client._summary({}) == "[AutoFix] Production incident auto-detected"
...
def test_create_issue_uses_configured_type_and_dynamic_summary(monkeypatch):
    ...
    monkeypatch.setattr(jira_client, "JIRA_PROJECT_KEY", "AutoFix")
    monkeypatch.setattr(jira_client, "JIRA_ISSUE_TYPE", "Task")
    captured = {}

    class FakeResp:
        def raise_for_status(self): pass
        def json(self): return {"key": "AutoFix-1"}
    ...
    assert key == "AutoFix-1"
    assert url == "https://x.atlassian.net/browse/AutoFix-1"
    f = captured["payload"]["fields"]
    assert f["project"]["key"] == "AutoFix"
    assert f["issuetype"]["name"] == "Task"
    assert f["summary"].startswith("[AutoFix]")
    assert "autofix" in f["labels"]
